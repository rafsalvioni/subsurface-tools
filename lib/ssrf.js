...
class DiveSites
{
    ...
    *[Symbol.iterator]()
    ...
    #createSite(point)
    {
        ...
        return this.constructor.#elToSite(el);
    }
}
...
class Dive
{
    ...
    getStart()
    {
        if (!this.#cache.start) {
            ...
            this.#cache.start = str.trim();
        }
        return new Date(this.#cache.start);
    }
    ...
    getEnd()
    {
        if (!this.#cache.end) {
            ...
            this.#cache.end = start.getTime() + dur;
        }
        return new Date(this.#cache.end);
    }
    ...
    getDuration()
    {
        if (!this.#cache.dur) {
            ...
            this.#cache.dur = timeToInt(time);
        }
        return this.#cache.dur;
    }
    ...
    getSpot()
    {
        ...
        if (!this.#cache.alt) {
            ...
            this.#cache.alt = Math.max(alt, barToMeters(bar));
        }

        let point = Object.assign(new Point(0,0), this.#site.point, {alt: this.#cache.alt});
        return point;
    }
