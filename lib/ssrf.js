...
export class DiveLog
{
    ...
    *[Symbol.iterator]()
    ...
    /**
     * Adds a Serial extradata to all DCs data from given model and that dont have one.
     * 
     * Returns DCs amount changed
     * 
     * @param {string} model 
     ...
     * @returns int
     */
    addDCSerial(model, serial)
    {
        // Get all DCs data from given model
        let dcs   = document.evaluate(
            `//divecomputer[@model='${model}' and @deviceid]`, this.#root,
            null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE
        );
        let count = 0;
        for (let i = 0; i < dcs.snapshotLength; i++) {
            ...
            if (document.evaluate("extradata[@key='Serial']", dc).iterateNext()) {
                continue; // DC have a serial... continue
            }
            dc.innerHTML += objToXml({'@key': 'Serial', '@value': serial}, 'extradata');
            count++;
        }
        return [dcs.snapshotLength, count];
    }
...
class DiveSites
{
    ...
    *[Symbol.iterator]()
     ...
     * @returns bool
     */
    setTimeZone(uuid, tz, replace=true)
    {
        if (!TZ_REGEX.test(tz)) {
            throw `Invalid tz: ${tz}`;
        }
        let site = this.#loadSite(uuid);
        if (site) {
            let obj   = domToStruct(site, 1);
            let tzTag = `#tz:${tz}`;
            if (obj.notes) {
                let notes = obj.notes[0]['*'];
                notes = notes.replace(TZTAG_REGEX, '').trim();
                if (replace || notes == obj.notes[0]['*']) {
                    notes += ` ${tzTag}`;
                    site.innerHTML = site.innerHTML.replace(/<notes>.+?<\/notes>(\r?\n)?/i, objToXml(notes, 'notes'));
                }
            }
            else {
                site.innerHTML += objToXml(tzTag, 'notes');
            }
            return true;
        }
        return false;
...
class Dive
{
    ...
    setTimeZone(tz, replace=true)
    {
        ...
        let tzTag = `#tz:${tz}`;
        if (tags) {
            let tags2 = tags.replace(TZTAG_REGEX, '');
            if (replace || tags == tags2) {
                tags2 += `, ${tzTag}`;
                tags   = tags2.replace(/( *, *){2,}/, ', ').replace(/^ *,/, '');
            }
            else {
                return this;
            }
    ...
    #loadTZ()
    {
        ...
        if (tags && (m = tags.match(TZTAG_REGEX))) { // Lets try in Dive's tags
            tz = m[1];
            this.#writeLog(`using own TZ ${tz}`);
        }
        else if (this.#site) { // Noop... Is there a site?
            let notes = this.#site.notes;
            if (notes && (m = notes.match(TZTAG_REGEX))) { // Is there TZ in site's notes?
                tz = m[1];
