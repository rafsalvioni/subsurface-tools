    ...
    setTimeZone(uuid, tz, replace=true)
    {
        ...
        if (site) {
            ...
            if (obj.notes) {
                ...
                notes = notes.replace(TAZTAG_REGEX, '').trim();
...
class Dive
{
    ...
    setTimeZone(tz, replace=true)
    {
        ...
        let tzTag = `#tz:${tz}`;
        if (tags) {
            let tags2 = tags.replace(TAZTAG_REGEX, '');
    ...
    #loadTZ()
    {
        ...
        if (tags && (m = tags.match(TAZTAG_REGEX))) { // Lets try in Dive's tags
            tz = m[1];
            this.#writeLog(`using own TZ ${tz}`);
        }
        else if (this.#site) { // Noop... Is there a site?
            let notes = this.#site.notes;
            if (notes && (m = notes.match(TAZTAG_REGEX))) { // Is there TZ in site's notes?
                tz = m[1];
