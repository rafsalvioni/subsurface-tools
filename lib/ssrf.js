    ...
    *[Symbol.iterator]()
    {
        ...
        for (const el of sites) {
            yield new Site(el, this.#diveLog);
        }
    }
    ...
    getByUuid(uuid)
    {
        ...
        if (site) {
            return new Site(site, this.#diveLog);
        }
        return null;
    }
    ...
    getByPosition(point, create=false)
    {
        ...
        for (let i = 0; i < sites.snapshotLength; i++) { // Lets refine... We are going to choose the nearest site
            ...
            let site = new Site(el, this.#diveLog);
...
class Site
{
    ...
    setTimeZone(tz, replace=true)
    {
        ...
        if (notes) {
            ...
            if (replace || notes == this.getNotes()) {
                notes += ` ${tzTag}`;
...
class Dive
{
    ...
    getTimeZoneUsed()
    {
        return this.#loadCache('tz', () => {
            ...
            else if (this.#site && (tz = this.#site.getTimeZone())) { // Noop... Is in dive's site?
